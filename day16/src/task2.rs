...
pub fn execute() {
    ...
    for i in 0..n {
        ans = i32::max(ans, find_ans(&contents, (i, -1), Direction::Right));
        ans = i32::max(ans, find_ans(&contents, (i, m), Direction::Left));
    }

    for i in 0..m {
        ans = i32::max(ans, find_ans(&contents, (-1, i), Direction::Down));
        ans = i32::max(ans, find_ans(&contents, (n, i), Direction::Up));
    }

    println!("{ans}");
}

fn find_ans(contents: &Vec<&str>, start: (i32, i32), dir: Direction) -> i32 {
    let mut rays: VecDeque<Ray> = VecDeque::new();

    let n = contents.len() as i32;
    let m = contents[0].len() as i32;

    rays.push_back(Ray{x: start.0, y: start.1, dir: dir});
